...
int IsExpression(int i, FILE* ef)
{
	...
	for (; (Data.LexTable[nom].type != LSeparator); nom++)
	{
		...
		if (Data.LexTable[nom].type == LRBraket)
			if (!(IsOperation(Data.LexTable[nom + 1].type)) && (Data.LexTable[nom + 1].type != LSeparator) && (Data.LexTable[nom + 1].type != LRBraket) && (Data.LexTable[nom + 1].type != LDo))
			{
...
int ErrorChecking()
{
	...
	{
		Err_num++;
		fprintf(ef, "line %d: \t'Int_2' expected!\n", Data.LexTable[3].line);
	}
	if (Data.LexTable[6].type == LVarType)
	{
		i = 7;
	...
	switch (Balans(0, LEOF, LBody, LEnd))
	{
	case 1:
		Err_num++;
	...
	for (j = 0;; j++)
	{
		...
		if (Data.LexTable[j].type == LDo)				//перевірка for
		{
			if (Data.LexTable[j + 1].type != LBody)
			{
				Err_num++;
